    ...
    public static void print(byte[] bytes) {
        ...
        for(int i =0;i<bytes.length;i++)
        {
           ...
           int valueLong = bytes[i] & 0xFF  ;
            ...
            if (Integer.toBinaryString(valueLong).toCharArray().length<8)
            {
                for (int j=0;j<8-Integer.toBinaryString(valueLong).toCharArray().length;j++)
                {
                    System.out.print("0");
                }
            }

            System.out.print(Integer.toBinaryString(valueLong));
            //String s =new String();
